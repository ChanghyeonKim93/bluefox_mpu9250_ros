...
void SerialCommunicatorLinux::read_withChecksum(char* msg){
...
void SerialCommunicatorLinux::processRX(std::shared_future<void> terminate_signal){
    ...
    stack_len_ = 0;
    // int timeout_poll = 1000; // ms
    int timeout_poll = -1; // ms
    ...
    while(true) {
        ...
        if (poll_state_ > 0) {
            if (poll_events_.revents & POLLIN) { // event is receiving data.
                    ...
                    for(int i = 0; i < len_read; ++i) {
                        ...
                        if(current_ascii == STX_){
                            ...
                            if(crc_calc == crc_recv) { // 'Checksum test' PASS, MESSAGE SUCCESSFULLY RECEIVED.
                                ...
                                std::cout << "RX time:" << std::setw(9) << (double)diff.count()*1000.0 << " [ms]\n";
                ...
                else std::cout << "______________________LEN <= 0\n\n";
            }
            ...
            printf("[SerialComm] RX - TIMEOUT-1000 [ms].\n");
        }
        ...
        // std::cerr << "WHILE LOOP TIME:" << std::setw(9) << (double)diff.count()*1000.0 << " [ms]\n";
...
void SerialCommunicatorLinux::processTX(std::shared_future<void> terminate_signal){
    while(true){
        ...
        if(len_send_ > 0){
            ...
            std::cout << " TX OK!:"<< ++seq_send_ <<", length: " << len_tmp << std::endl;
            len_send_ = 0;
        }
        mutex_tx_->unlock();
